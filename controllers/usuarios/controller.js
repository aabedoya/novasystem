...
const consultarOCrearUsuario = async (req, callback) => {
  ...
  await baseDeDatos.collection('usuarios').findOne({ email: user.email }, async (err, response) => {
    ...
    if (response) {
      console.log('Esta creado');// 7.1. si el usuario ya esta en la BD, devuelve la info del usuario
      callback(err, response);
    } else {
      console.log('Deberìa crearlo'); // 7.2. si el usuario no esta en la bd, lo crea y devuelve la info
